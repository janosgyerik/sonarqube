...
public class LogsAction implements SystemWsAction {
  ...
  public void define(WebService.NewController controller) {
    ...
    action
      ...
      .setPossibleValues(stream(LogProcess.values())
        .map(LogProcess::getProcessName)
        .sorted()
        .collect(Collectors.toList(LogProcess.values().length)))
      .setDefaultValue(LogProcess.APP.getProcessName())
      .setSince("6.2")
  ...
  public void handle(Request wsRequest, Response wsResponse) throws Exception {
    ...
    String processName = wsRequest.mandatoryParam(PROCESS_PROPERTY);
    LogProcess process = LogProcess.fromProcessName(processName);

    wsResponse.stream().setMediaType(MediaTypes.TXT);
    File logsDir = serverLogging.getLogsDir();
    File file = new File(logsDir, process.getFileNamePrefix() + ".log");
